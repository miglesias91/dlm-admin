...
mongoexport --host=${ip} --port=27017 --authenticationDatabase ${authdb} -u ${usuario} -p ${password} --db=${db_noticias} --collection=${coleccion_noticias} --query='{"fecha":{"$gte":{"$date":"'${ayer}'T00:00:00.000Z"}, "$lte":{"$date":"'${ayer}'T23:59:59.000Z"}}}' --type csv --fields diario,seccion,fecha,titulo,texto  --out ${2}noticias/${ayer}.csv
